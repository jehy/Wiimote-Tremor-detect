...
namespace WiimoteTest
{
	public partial class WiimoteInfo : UserControl
	{
        ...
        public WiimoteLib.ButtonState PrevButtonState;

        public String buffer = "";
        public String FileName = "";
        int WorkStatus = 0;

        public DateTime msec;
        public DateTime msec_last = DateTime.Now;

        private void WriteLog(int mode)
        {
            if ((buffer.Length > 10240) || (mode == 1))
            {
		...
		{
			InitializeComponent();
            PrevButtonState.A=false;
            PrevButtonState.B=false;
            PrevButtonState.Down=false;
            PrevButtonState.Home=false;
            PrevButtonState.Left=false;
            PrevButtonState.Minus = false;
            PrevButtonState.One = false;
            PrevButtonState.Plus = false;
            PrevButtonState.Right = false;
            PrevButtonState.Two = false;
            PrevButtonState.Up = false;
            b= new Bitmap(pbIR.Width, pbIR.Height, PixelFormat.Format24bppRgb);
		...
		private void UpdateWiimoteChanged(WiimoteChangedEventArgs args)
		{
            ...
            if (PrevButtonState.A != ws.ButtonState.A)
            {
                clbButtons.SetItemChecked(0, ws.ButtonState.A);
                PrevButtonState.A = ws.ButtonState.A;
                SendKeys.Send("{ENTER}");
            }

			clbButtons.SetItemChecked(1, ws.ButtonState.B);

            if (PrevButtonState.Minus != ws.ButtonState.Minus)
            {
                clbButtons.SetItemChecked(2, ws.ButtonState.Minus);
                PrevButtonState.Minus = ws.ButtonState.Minus;
                Afkvalley.WinAPI.MouseClick("left", !ws.ButtonState.Minus);

            }
            if (PrevButtonState.Home != ws.ButtonState.Home)
            {
                clbButtons.SetItemChecked(3, ws.ButtonState.Home);
                PrevButtonState.Home = ws.ButtonState.Home;
                Afkvalley.WinAPI.MouseClick("middle", !ws.ButtonState.Home);
            }

            if (PrevButtonState.Plus != ws.ButtonState.Plus)
            {
                clbButtons.SetItemChecked(4, ws.ButtonState.Plus);
                PrevButtonState.Plus = ws.ButtonState.Plus;
                Afkvalley.WinAPI.MouseClick("right", !ws.ButtonState.Plus);
            }
            ...
            if (PrevButtonState.Up != ws.ButtonState.Up)
            {
                clbButtons.SetItemChecked(7, ws.ButtonState.Up);
                //PrevButtonState.Up = ws.ButtonState.Up;
                SendKeys.Send("{UP}");
                System.Threading.Thread.Sleep(300);
            }
            if (PrevButtonState.Down != ws.ButtonState.Down)
            {
                clbButtons.SetItemChecked(8, ws.ButtonState.Down);
                //PrevButtonState.Down = ws.ButtonState.Down;
                SendKeys.Send("{DOWN}");
                System.Threading.Thread.Sleep(300);

            }
            if (PrevButtonState.Left != ws.ButtonState.Left)
            {
                clbButtons.SetItemChecked(9, ws.ButtonState.Left);
                //PrevButtonState.Left = ws.ButtonState.Left;
                SendKeys.Send("{LEFT}");
                System.Threading.Thread.Sleep(300);
            }
            if (PrevButtonState.Right != ws.ButtonState.Right)
            {
                clbButtons.SetItemChecked(10, ws.ButtonState.Right);
                //PrevButtonState.Right = ws.ButtonState.Right;
                SendKeys.Send("{RIGHT}");
                System.Threading.Thread.Sleep(300);
            }
            ...
            if (ws.IRState.IRSensors[0].Found && ws.IRState.IRSensors[1].Found)
            {
                ...
                Cursor.Position = new System.Drawing.Point((int)(ws.IRState.RawMidpoint.X / 4), (int)(ws.IRState.RawMidpoint.Y / 4));
            
            }

            //pbIR.SetBounds(0, 0, pbIR.Width, pbIR.Height);
			pbIR.Image = b;
		...
		private void UpdateIR(IRSensor irSensor, Label lblNorm, Label lblRaw, CheckBox chkFound, Color color)
		{
            ...
            g.DrawEllipse(new Pen(color), (int)(pbIR.Width / 2), (int)(pbIR.Height / 2), 4, 4);
			...
			if(irSensor.Found)
			{
                ...
                {
                    buffer += "\n";
                    long time_elapsed = ((DateTime.Now.Ticks - msec.Ticks) / 10000);//milliseconds
                    long time_elapsed_last = ((DateTime.Now.Ticks - msec_last.Ticks) / 10000);//milliseconds
                    buffer += time_elapsed.ToString();
                    buffer += " " + (irSensor.RawPosition.X).ToString() + " " + irSensor.RawPosition.Y.ToString();


                    msec_last = DateTime.Now;
                    WriteLog(0);

                }
				lblNorm.Text = irSensor.Position.ToString() + ", " + irSensor.Size;
				lblRaw.Text = irSensor.RawPosition.ToString();
                //g.DrawEllipse(new Pen(color), (int)(irSensor.RawPosition.X / 4), (int)(irSensor.RawPosition.Y / 4), irSensor.Size+1, irSensor.Size+1);
                g.DrawEllipse(new Pen(color), (int)pbIR.Width - (int)(irSensor.RawPosition.X / 2), (int)(irSensor.RawPosition.Y / 2), irSensor.Size + 4, irSensor.Size + 4);

                //Cursor.Position = new System.Drawing.Point((int)(irSensor.RawPosition.X / 4), (int)(irSensor.RawPosition.Y / 4));
                //System.Windows.Forms.MouseButtons.l
			}
		}
        ...
        private void start_Click(object sender, EventArgs e)
        {
            ...
            msec = DateTime.Now;
            int i = 0;
            while (File.Exists("data_" + i.ToString() + ".txt"))
                i++;
            FileName = "data_" + i.ToString() + ".txt";
            WorkStatus = 1;
        }
